...
class Model(nn.Module):
    def __init__(self, n_in, n_h, activation, negsamp_round, readout, have_neg = False, hidden_size = 128,
                 temperature=0.4, lamb=20, is_rectified=True, topo_t=2, neg_top_k=50):
        ...
        self.fc1 = nn.Linear(2*n_h, n_h)
        self.fc2 = nn.Linear(2*n_h, n_h)
        if readout == 'max':
    ...
    def node_node(self, h1, h2, neg_top_k = 50):
        ...
        neg_sim = neg_sim.to(torch.device('cuda:'+str(args.cuda) if torch.cuda.is_available() else 'cpu'))
    ...
    def forward(self, feature1, adj1, raw1, size1, feature2, adj2, raw2, size2,
        ...
        # --------------------node-subgraph alignment loss---------------#
        intra_cat_1 = torch.cat((h_node_1, h_graph_read_1), dim=1)
        intra_cat_2 = torch.cat((h_node_2, h_graph_read_2), dim=1)
        intra_cat_11 = self.fc1(intra_cat_1)
        intra_cat_22 = self.fc2(intra_cat_2)
        intra_ave_1 = (h_node_1 + h_graph_read_1) / 2
        intra_ave_2 = (h_node_2 + h_graph_read_2) / 2
        deep_loss_1 = torch.mean(F.cosine_similarity(intra_cat_11, intra_ave_1, dim=1))
        deep_loss_2 = torch.mean(F.cosine_similarity(intra_cat_22, intra_ave_2, dim=1))
        ...
        return deep_loss_1, deep_loss_2, node_recons_1, node_recons_2, disc_1, disc_2, Intra_loss_1, Intra_loss_2, sim_all_node1, sim_pos_node1, sim_all_node2, sim_pos_node2, h1[:, -1, :], h2[:, -1, :]
...
model = Model(n_in=ft_size, n_h=args.embedding_dim, activation='prelu', negsamp_round=args.negsamp_ratio,
            ...
            lamb=args.Sinkhorn_lamb, is_rectified=args.rectified,
...
with tqdm(total=args.epochs) as pbar:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx in range(batch_num):
            ...
            #/---------------------MODEL-----------------------/#
            deep_loss_1, deep_loss_2, node_recons_1, node_recons_2, disc_1, disc_2, Intra_loss_1, Intra_loss_2, _, _, _, _, _, _ = \
            ...
            deep_loss = (deep_loss_1 + deep_loss_2) / 2
            loss_node = torch.mean((Intra_loss_1 + Intra_loss_2) / 2)
            loss = alpha_recon * loss_recon + alpha_intra * loss_intra + loss_node + deep_loss * alpha_inter
        ...
        if mean_loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), 'pkl/model' + args.dataset + str(args.K_1) + str(args.K_2) + '.pkl')
        else:
            cnt_wait += 1
        pbar.set_postfix(loss=mean_loss)
        pbar.update(1)
path = 'pkl/model' + args.dataset + str(args.K_1) + str(args.K_2) + '.pkl'
model.load_state_dict(torch.load(path))
multi_round_attr_ano_score = np.zeros((args.auc_test_rounds, nb_nodes))
# batch批次个元素
nodes_embed1 = torch.zeros([nb_nodes, args.embedding_dim], dtype=torch.float, device=device)
nodes_embed2 = torch.zeros([nb_nodes, args.embedding_dim], dtype=torch.float, device=device)
...
with tqdm(total=args.auc_test_rounds) as pbar_test:
    ...
    for round in range(args.auc_test_rounds):
        ...
        for batch_idx in range(batch_num):
            ...
            # /---------------------MODEL-----------------------/#
            with torch.no_grad():
                deep_score_1, deep_score_2, node_res_1, node_res_2, logits_1, logits_2, Intra_loss_1, Intra_loss_2, sim_all_node1, sim_pos_node1, \
                    sim_all_node2, sim_pos_node2, batch_embed1, batch_embed2 = model(bf, ba, raw, subgraph_size_1 - 1, bf_2, ba_2, raw_2, subgraph_size_2 - 1,
                                                full_topology_dist, subgraph_idx, subgraph_idx_2)
            ...
            score_deep = ((deep_score_1 + deep_score_2) / 2).cpu().numpy()
            ...
            ano_score_deep = scaler3.fit_transform(score_deep.reshape(-1, 1)).reshape(-1)
            ano_score_otx = scaler4.fit_transform(score_otx.reshape(-1, 1)).reshape(-1)
            ano_scores = alpha_intra * ano_score_co + alpha_recon * ano_score_re + ano_score_otx + alpha_inter * ano_score_deep  # anomaly score have ot(pos)
